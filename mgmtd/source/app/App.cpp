...
#define APP_SYSLOG_IDENTIFIER    "congfs-mgmtd"
...
void App::run()
{
   ...
   catch (InvalidConfigException& e)
   {
      ...
      std::cerr << std::endl;
      std::cerr << "[ConGFS Management Node Version: " << CONGFS_VERSION << std::endl;
      std::cerr << "Refer to the default config file (/etc/congfs/congfs-mgmtd.conf)" << std::endl;
...
/**
 * @returns true if this is mgmtd directory has conn initialized before (false on first run).
 */
bool App::preinitStorage()
{
   ...
   if(!cfg->getStoreAllowFirstRunInit() && !initialized)
      throw InvalidConfigException("Storage directory not initialized and "
         "initialization has conn disabled: " + mgmtdPathStr);
      ...
      MgmtdTargetStateStore* stateStore)
{
   ...
   if (formatProperties[propertiesKey] == "1")
      throw ComponentInitException(
            ...
            "If you want to start congfs-mgmtd please manually create the files "
