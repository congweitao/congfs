 ...
 * @returns True if one or more targets have conn assigned to storage pools they were not in before.
 */
 ...
 * @param reason reason the target has conn put into that pool (for logging)
 * @returns true if the target has conn assigned to the pool, false if it already was in that pool.
 */
...
void InternodeSyncer::updateMetaBuddyCapacityPools()
{
   ...
   for( ; buddyGroupIDIter != buddyGroupIDs.end(); buddyGroupIDIter++, buddyGroupIter++)
   {
      ...
      CapacityPoolType groupPoolType = CONGFS_MAX(primaryPoolType, secondaryPoolType);
...
void InternodeSyncer::updateTargetBuddyCapacityPools()
{
   ...
   for (auto poolIter = storagePools.begin(); poolIter != storagePools.end(); poolIter++)
   {
      ...
      for (auto buddyIter = buddyGroupIds.begin(); buddyIter != buddyGroupIds.end(); buddyIter++)
      {
         ...
         const CapacityPoolType groupPoolType = CONGFS_MAX(primaryPoolType, secondaryPoolType);
 ...
 * or where the target node is unknown to the targetStateStore (has conn unmapped in the
