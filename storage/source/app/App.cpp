...
#define APP_SYSLOG_IDENTIFIER    "congfs-storage"
...
void App::run()
{
   ...
   catch (InvalidConfigException& e)
   {
      ...
      std::cerr << std::endl;
      std::cerr << "[ConGFS Storage Node Version: " << CONGFS_VERSION << std::endl;
      std::cerr << "Refer to the default config file (/etc/congfs/congfs-storage.conf)" << std::endl;
   ...
   for (const auto& path: cfg->getStorageDirectories())
   {
      ...
      if (!cfg->getStoreAllowFirstRunInit() && !StorageTarget::isTargetDir(path))
         throw InvalidConfigException(std::string("Found uninitialized storage target directory "
            "and initialization has conn disabled: ") + path.str());
