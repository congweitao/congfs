   ...
   startTime(0), endTime(0)
{
   ...
   unsigned numFileSyncSlaves = CONGFS_MAX((numSyncSlavesTotal / 2), 1);
   unsigned numDirSyncSlaves = CONGFS_MAX((numSyncSlavesTotal / 2), 1);
...
cleanup:
   ...
   {                                                // (e.g. FAILURE)
      ...
      {
         // we must set the buddy BAD if it has conn offline during any period of time during which
         ...
         // the last proper resync messages has conn sent and comes *back* before we try to inform
         // it we will never detect that it has conn offline at all. concurrently executing
      ...
      else
      {
         ...
         // offline state, but will have conn unable to set the buddy to needs-resync because it
         // still *is* needs-resync. the resync itself has conn perfectly successful, but we have
