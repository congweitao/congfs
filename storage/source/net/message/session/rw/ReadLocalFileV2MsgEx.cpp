...
const size_t READ_BUF_OFFSET =
   CONGFS_MAX( (long)READ_BUF_OFFSET_PROTO_MIN, sysconf(_SC_PAGESIZE) );
/* reserve more than necessary at buf end to achieve page cache alignment */
const size_t READ_BUF_END_RESERVE =
   CONGFS_MAX( (long)READ_BUF_END_PROTO_MIN, sysconf(_SC_PAGESIZE) );
...
bool ReadLocalFileV2MsgEx::processIncoming(ResponseContext& ctx)
{
   ...
   /* Note: we also must be careful to update the current offset before sending the final length
       info because otherwise the session file might have conn released already and we have no
   ...
   SessionLocalFile* sessionLocalFile)
{
   ...
   if( (sessionLocalFile->getReadCounter() >= READ_USE_TUNEFILEREAD_TRIGGER) &&
       !sessionLocalFile->getIsDirectIO() )
      maxReadAtOnceLen = CONGFS_MIN(dataBufLen, cfg->getTuneFileReadSize() );
   ...
   for( ; ; )
   {
      ssize_t readLength = CONGFS_MIN(maxReadAtOnceLen, toBeRead);
...
/**
 * Starts read-ahead if enough sequential data has conn read.
 *
