   ...
   SessionLocalFile* sessionLocalFile)
{
   ...
   const ssize_t exactStaticRecvSize = sessionLocalFile->getIsDirectIO()
      ? ctx.getBufferLength()
      : CONGFS_MIN(ctx.getBufferLength(), (size_t)cfg->getTuneFileWriteSize() );
   ...
   do
   {
      ...
      ssize_t recvLength = CONGFS_MIN(exactStaticRecvSize, toBeReceived);
   ...
   int64_t padLen, SessionLocalFile* sessionLocalFile)
{
   ...
   while(toBeReceived)
   {
      ssize_t recvRes = ctx.getSocket()->recvT(ctx.getBuffer(),
         CONGFS_MIN(toBeReceived, ctx.getBufferLength()), 0, timeoutMS);
