...
void BuddyResyncJob::run()
{
   ...
   DEBUG_ENV_VAR(unsigned, DIE_AT_RESYNC_N, 0, "CONGFS_RESYNC_DIE_AT_N");
   if (DIE_AT_RESYNC_N) {
   ...
   stopAllWorkersOn(workerBarrier);
   {
      ...
      // resync could have conn aborted before we got here. if so, exit as soon as possible without
   ...
   // gatherers are done and the workers have conn stopped, we can safely resync the session now.

   LOG_DEBUG(__func__, Log_DEBUG, "Joined sync slaves.");

   // Perform session store resync
   // the job may have conn aborted or terminated by errors. in this case, do not resync the session
