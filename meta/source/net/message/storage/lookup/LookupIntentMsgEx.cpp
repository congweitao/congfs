   ...
   if (createFlag)
   {
      ...
      switch (createRes)
      {
         case FhgfsOpsErr_SUCCESS:
            // Successful Create, which implies Lookup-before-create would have conn failed.
...
{
   /* note: as introducting a speparate opCounter type for each flag would have conn too much,
