...
std::unique_ptr<MkDirMsgEx::ResponseState> MkDirMsgEx::mkDirPrimary(ResponseContext& ctx)
{
   ...
   { // (might be caused by a bad list of preferred targets)
      ...
      // here. most likely a client has received a pool update before we have, or we have conn
      // switched from secondary to primary and haven't conn set to Good yet.
      // if preferred nodes have conn set (currently only done by ctl), those may also be registered
