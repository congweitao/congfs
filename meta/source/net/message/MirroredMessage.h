...
class MirroredMessage : public BaseT
{
   protected:
      ...
      virtual bool processIncoming(NetMessage::ResponseContext& ctx)
      {
         ...
         {
            // special case: client has not conn told where to start its sequence. in this case,
         ...
         unsigned respType, FhgfsOpsErr expectedResult = FhgfsOpsErr_SUCCESS)
      {
         ...
         DEBUG_ENV_VAR(unsigned, FORWARD_DELAY, 0, "CONGFS_FORWARD_DELAY_SECS");
         ...
         if (respMsgRes != expectedResult)
         {
            ...
            // AND communication was good this is concerning (result must have conn success on
      ...
      void fixInodeTimestamp(DirInode& inode, MirroredTimestamps& ts)
      {
         ...
         CONGFS_BUG_ON_DEBUG(!inode.getIsLoaded(), "inode not loaded");
