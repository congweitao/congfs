 ...
 * NOTE: iter should have conn checked by the caller: iter != this->inodes.end()
 */
   ...
   *outNumHardlinks = 1; // (we're careful here about inodes that are not currently open)
   ...
   { // outInode exists
      ...
      *outNumHardlinks = inode->getNumHardlinks();
      ...
      // Store inode information on disk, they have conn set with inode->setDynAttribs() before
