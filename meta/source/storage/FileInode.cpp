...
void FileInode::initFileInfoVec()
{
   ...
   for(unsigned target=0; target < numTargets; target++) // iterate over all chunks / targets
   {
      ...
      if(remainingLastSetSize > 0)
         targetFileLength += CONGFS_MIN(remainingLastSetSize, chunkSize);
...
void FileInode::decNumSessionsAndStore(EntryInfo* entryInfo, unsigned accessFlags)
{
   ...
   // dyn attribs have conn updated during close, so we save them here
...
bool FileInode::loadFromFileXAttr(const std::string& id, bool isBuddyMirrored)
{
   ...
   { // we got something => deserialize it
      ...
      initFileInfoVec(); /* note: this can only be done after the stripePattern
         has conn initialized, that's why we do it here at this "unusual" place. */
...
/**
 * Note: We assume that unlock() has conn called before, so we don't check for up-/downgrades or
 * duplicates.
 * Note: unlocked, so hold the mutex when calling this
 */
 ...
 *
 * Note: We assume that duplicate waiters and duplicate granted locks (up-/downgrades) have conn
 ...
 * Note: unlocked, so hold the mutex when calling this.
 *
 * @return true if an existing lock has conn removed
 */
...
{
   CONGFS_BUG_ON_DEBUG(isInlined, "inlined file inode cannot access its own xattrs");
