...
int main(int argc, char** argv)
{
   ...
   // check if the file is located on a ConGFS, because the striping API works only on a ConGFS
   // (Results of ConGFS ioctls on other file systems are undefined.)
   bool isBeegfs = congfs_testIsConGFS(fd);
   if(!isBeegfs)
   {
      std::cout << "The given file is not located on an ConGFS: " << file << std::endl;
      exit(-1);
   }
   ...
   bool stripeInfoRetVal = congfs_getStripeInfo(fd, &outPatternType, &outChunkSize, &outNumTargets);
   if(stripeInfoRetVal)
   {
      std::string patternType;
      switch(outPatternType)
      {
         case CONGFS_STRIPEPATTERN_RAID0:
            patternType = "RAID0";
            break;
         case CONGFS_STRIPEPATTERN_RAID10:
            patternType = "RAID10";
            break;
         case CONGFS_STRIPEPATTERN_BUDDYMIRROR:
      ...
      for (int targetIndex = 0; targetIndex < outNumTargets; targetIndex++)
      {
         ...
         bool stripeTargetRetVal = congfs_getStripeTargetV2(fd, targetIndex, &outTargetInfo);
         if(stripeTargetRetVal)
         {
            if(outPatternType == CONGFS_STRIPEPATTERN_BUDDYMIRROR)
            {
