#ifndef __CONGFS_IOCTL_FUNCTIONS_H__
#define __CONGFS_IOCTL_FUNCTIONS_H__
...
#define CONGFS_API_MAJOR_VERSION 1 // major version number of the API, different major version
                                   // are  incompatible
#define CONGFS_API_MINOR_VERSION 1 // minor version number of the API, the minor versions of the
                                   // same major version are backward compatible

#define congfs_api_version_check() { return congfs_checkApiVersion(); } // backward compatibility


static inline bool congfs_getConfigFile(int fd, char** outCfgFile);
static inline bool congfs_getRuntimeConfigFile(int fd, char** outCfgFile);
static inline bool congfs_testIsConGFS(int fd);
static inline bool congfs_getMountID(int fd, char** outMountID);
static inline bool congfs_getStripeInfo(int fd, unsigned* outPatternType, unsigned* outChunkSize, 
   uint16_t* outNumTargets);
static inline bool congfs_getStripeTarget(int fd, uint16_t targetIndex, uint16_t* outTargetNumID, 
   uint16_t* outNodeNumID, char** outNodeStrID);
static inline bool congfs_getStripeTargetV2(int fd, uint32_t targetIndex,
   struct BeegfsIoctl_GetStripeTargetV2_Arg* outTargetInfo);
static inline bool congfs_createFile(int fd, const char* filename, mode_t mode,
   unsigned numtargets, unsigned chunksize);
static inline bool congfs_getInodeID(int fd, const char* entryID, uint64_t* outInodeID);
static inline bool congfs_getEntryInfo(int fd, uint32_t* ownerID, char* parentEntryID,
      char* entryID, int* entryType, int* featureFlags);
static inline bool congfs_checkApiVersion(const unsigned required_major_version,
   const unsigned required_minor_version);


/**
 * Get the path to the client config file of an active ConGFS mountpoint.
 * 
 * @param fd filedescriptor pointing to file or dir inside ConGFS mountpoint.
 ...
 */
bool congfs_getConfigFile(int fd, char** outCfgFile)
{
   struct BeegfsIoctl_GetCfgFile_Arg getCfgFile;
   getCfgFile.length = CONGFS_IOCTL_CFG_MAX_PATH;

   int res = ioctl(fd, CONGFS_IOC_GET_CFG_FILE, &getCfgFile);
   if(res)
      return false;

   *outCfgFile = strndup(getCfgFile.path, CONGFS_IOCTL_CFG_MAX_PATH);
   if(!*outCfgFile)
      return false;
 ...
 * @return true on success, false on error (in which case errno will be set).
 */
bool congfs_getRuntimeConfigFile(int fd, char** outCfgFile)
{
   struct BeegfsIoctl_GetCfgFile_Arg getCfgFile;
   getCfgFile.length = CONGFS_IOCTL_CFG_MAX_PATH;

   int res = ioctl(fd, CONGFS_IOC_GET_RUNTIME_CFG_FILE, &getCfgFile);
   if(res)
      return false;
...
/**
 * Test if the underlying file system is a ConGFS.
 * 
 * @param fd filedescriptor pointing to some file or dir that should be checked for whether it is
 *        located inside a ConGFS mount.
 * @return true on success, false on error (in which case errno will be set).
 */
bool congfs_testIsConGFS(int fd)
{
   char testArray[sizeof(CONGFS_IOCTL_TEST_STRING)];

#ifdef CONGFS_DEBUG
   // just calm valgrind; it does not detect that the array is initialized by the ioctl
   memset(testArray, 0, sizeof(CONGFS_IOCTL_TEST_STRING) );
#endif

   int ioctlRes = ioctl(fd, CONGFS_IOC_TEST_IS_CONGFS, testArray);
   if(ioctlRes)
      return false;

   int memCmpRes = memcmp(testArray, CONGFS_IOCTL_TEST_STRING, sizeof(CONGFS_IOCTL_TEST_STRING) );
   if(memCmpRes)
   { // ioctl was accepted by underlying fs, but buffer wasn't filled correctly
      errno = EPROTO;
      return false; // verification through buffer failed, probably just not a congfs
   }
 ...
 * @return true on success, false on error (in which case errno will be set).
 */
bool congfs_getMountID(int fd, char** outMountID)
{
   char mountIDBuf[CONGFS_IOCTL_MOUNTID_BUFLEN];
   ...
   int ioctlRes = ioctl(fd, CONGFS_IOC_GET_MOUNTID, mountIDBuf);
   if(ioctlRes)
      return false;
 ...
 * 
 * @param fd filedescriptor pointing to some file inside a ConGFS mount.
 * @param outPatternType type of stripe pattern (CONGFS_STRIPEPATTERN_...)
 * @param outChunkSize chunk size for striping.
 * @param outNumTargets number of targets for striping.
 * @return true on success, false on error (in which case errno will be set).
 */
bool congfs_getStripeInfo(int fd, unsigned* outPatternType, unsigned* outChunkSize, 
   uint16_t* outNumTargets)
{
   struct BeegfsIoctl_GetStripeInfo_Arg getStripeInfo;
   
   int res = ioctl(fd, CONGFS_IOC_GET_STRIPEINFO, &getStripeInfo);
   if(res)
      return false;
 ...
 * @return true on success, false on error (in which case errno will be set).
 */
bool congfs_getStripeTarget(int fd, uint16_t targetIndex, uint16_t* outTargetNumID, 
   uint16_t* outNodeNumID, char** outNodeStrID)
{
   ...
   int res = ioctl(fd, CONGFS_IOC_GET_STRIPETARGET, &getStripeTarget);
   if(res)
      return false;
   ...
   *outNodeStrID = strndup(getStripeTarget.outNodeStrID, CONGFS_IOCTL_CFG_MAX_PATH);
   if(!*outNodeStrID)
      return false;
 ...
 * @return true on success, false on error (in which case errno will be set).
 */
bool congfs_getStripeTargetV2(int fd, uint32_t targetIndex,
   struct BeegfsIoctl_GetStripeTargetV2_Arg* outTargetInfo)
{
   ...
   return ioctl(fd, CONGFS_IOC_GET_STRIPETARGET_V2, outTargetInfo) == 0;
}
 ...
 * @return true on success, false on error (in which case errno will be set).
 */
bool congfs_createFile(int fd, const char* filename, mode_t mode, unsigned numtargets,
   unsigned chunksize)
{
   ...
   int res = ioctl(fd, CONGFS_IOC_MKFILE_STRIPEHINTS, &createFileArg);
   if(res)
      return false;
...
bool congfs_getInodeID(int fd, const char* entryID, uint64_t* outInodeID)
{
   struct BeegfsIoctl_GetInodeID_Arg getInodeIDArg;

   getInodeIDArg.entryID[CONGFS_IOCTL_ENTRYID_MAXLEN] = '\0';
   strncpy(getInodeIDArg.entryID, entryID, CONGFS_IOCTL_ENTRYID_MAXLEN);

   if(ioctl(fd, CONGFS_IOC_GETINODEID, &getInodeIDArg))
   {
      *outInodeID = 0;
      return false;
   }
 ...
 * be at least CONGFS_IOCTL_ENTRYID_MAXLEN + 1 bytes long.
 ...
 */
bool congfs_getEntryInfo(int fd, uint32_t* ownerID, char* parentEntryID,
      char* entryID, int* entryType, int* featureFlags)
{
   struct BeegfsIoctl_GetEntryInfo_Arg arg;

   if(ioctl(fd, CONGFS_IOC_GETENTRYINFO, &arg))
   {
      return false;
   }

   *ownerID = arg.ownerID;
   strncpy(parentEntryID, arg.parentEntryID, CONGFS_IOCTL_ENTRYID_MAXLEN + 1);
   strncpy(entryID, arg.entryID, CONGFS_IOCTL_ENTRYID_MAXLEN + 1);
 ...
 */
bool congfs_checkApiVersion(const unsigned required_major_version,
   const unsigned required_minor_version)
{
   if(required_major_version != CONGFS_API_MAJOR_VERSION)
      return false;

   if(required_minor_version > CONGFS_API_MINOR_VERSION)
      return false;

   return true;
}

#endif /* __CONGFS_IOCTL_FUNCTIONS_H__ */
