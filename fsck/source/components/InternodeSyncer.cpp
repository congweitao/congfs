   ...
   NumNodeIDList& removedNodes)
{
   ...
   if (!addedNodes.empty())
      LogContext(logContext).log(Log_WARNING,
         std::string("Nodes added while congfs-fsck was running: ")
            + StringTk::uintToStr(addedNodes.size())
            + " (Type: " + boost::lexical_cast<std::string>(nodeType) + ")");
   ...
   if (!removedNodes.empty())
   {
      ...
      LogContext(logContext).log(Log_WARNING,
         std::string("Nodes removed while congfs-fsck was running: ")
            + StringTk::uintToStr(removedNodes.size())
            + " (Type: " + boost::lexical_cast<std::string>(nodeType) + ")");
...
void InternodeSyncer::handleTargetMappingChanges()
{
      ...
      originalMapIter != originalTargetMap.end(); originalMapIter++ )
   {
      ...
      {
         LogContext(logContext).log(Log_WARNING,
            "Target removed while congfs-fsck was running; congfs-fsck can't continue; targetID: "
      ...
      else
      {
         ...
         {
            LogContext(logContext).log(Log_WARNING,
               "Target re-mapped while congfs-fsck was running; congfs-fsck can't continue; "
...
void InternodeSyncer::handleBuddyGroupChanges()
{
      ...
      originalMapIter != originalMirrorBuddyGroupMap.end(); originalMapIter++ )
   {
      ...
      {
         LogContext(logContext).log(Log_WARNING,
            "Mirror buddy group removed while congfs-fsck was running; congfs-fsck can't continue; "
      ...
      else
      {
         ...
         {
            LogContext(logContext).log(Log_WARNING,
               "Primary of mirror buddy group changed while congfs-fsck was running; congfs-fsck "
...
/**
 * Blocks until list of servers has conn downloaded from management node
 */
