...
void ModeCheckFS::printHelp()
{
   std::cout <<
      ...
      "\n"
      "USAGE:\n"
      " This mode performs a full check and optional repair of a ConGFS file system\n"
      " instance by building a database of the current file system contents on the\n"
      " local machine.\n"
      "\n"
      " The fsck gathers information from all ConGFS server daemons in parallel through\n"
...
int ModeCheckFS::execute()
{
   ...
   else
   {
         ...
         if (startLogRes != FhgfsOpsErr_SUCCESS)
         {
            ...
            switch (startLogRes)
            {
               case FhgfsOpsErr_INUSE:
                  errStr = "Another instance of congfs-fsck is still running or was aborted. "
                     "Cannot start a new one unless --forceRestart command line switch is used. "
                     "Do this only after making sure there is no other instance of confs-fsck "
                     "running.";
      ...
      if (!cfg->getRunOffline())
      {
         ...
         // if event logging is not OK (i.e. that not all events might have conn processed), go
         // into read-only mode
         if ( !eventLoggingOK )
         {
            ...
            FsckTkEx::fsckOutput(
               ...
               OutputOptions_FLUSH | OutputOptions_LINEBREAK);
            FsckTkEx::fsckOutput(
               "This means congfs-fsck is not aware of all changes in the filesystem and it is "
            ...
            FsckTkEx::fsckOutput(
               ...
               "You can still force repair by running congfs-fsck again on the existing "
...
void ModeCheckFS::printHeaderInformation()
{
   ...
   FsckTkEx::fsckOutput(
      "Started ConGFS fsck in forward check mode [" + timeStr.substr(0, timeStr.length() - 1)
   ...
   if (Program::getApp()->getMetaMirrorBuddyGroupMapper()->getSize() > 0)
   {
      FsckTkEx::fsckOutput(
            ...
            "primary. \n\nIf congfs-fsck is aborted prematurely, a resync needs to be initiated "
...
void ModeCheckFS::checkAndRepair()
{
   ...
   if(errorCount)
   {
      FsckTkEx::fsckOutput("Found errors congfs-fsck cannot fix. Please consult the log for "
