   ...
   const char* context, const char* msg)
{
   ...
   char threadName[CONGFS_TASK_COMM_LEN];
   LogMsg requestMsg;
   char* respBuf;
   NetMessage* respMsg;
...
void __Logger_logReachableLoggerd(Logger* this)
{
   ...
   else
   {
      ...
      printk_fhgfs(KERN_NOTICE, "External logger (provided by congfs-helperd) is "
...
void __Logger_logUnreachableLoggerd(Logger* this)
{
   ...
   {
      // print error message
      printk_fhgfs(KERN_WARNING, "External logger (provided by congfs-helperd) is "
