...
void Thread_init(Thread* this, const char* threadName, ThreadRoutine threadRoutine)
{
   ...
   StringTk_strncpyTerminated(this->initialThreadName, threadName, CONGFS_TASK_COMM_LEN);
 ...
 *
 * @param outBuf must be at least of length CONGFS_TASK_COMM_LEN
 */
void Thread_getCurrentThreadName(char* outBuf)
{
   StringTk_strncpyTerminated(outBuf, current->comm, CONGFS_TASK_COMM_LEN);
}
