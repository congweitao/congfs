...
#ifndef CONGFS_MODULE_NAME_STR
   #define CONGFS_MODULE_NAME_STR          "congfs"
#endif

#define CONGFS_THREAD_NAME_PREFIX_STR   CONGFS_MODULE_NAME_STR "_"

// a printk-version that adds the module name and comm name
#define printk_fhgfs(levelStr, fmtStr, ...) \
   printk(levelStr CONGFS_MODULE_NAME_STR ": " "%s(%u): " fmtStr, current->comm, \
...
#define printk_fhgfs_ir(levelStr, fmtStr, ...) \
   printk(levelStr CONGFS_MODULE_NAME_STR ": " fmtStr, ## __VA_ARGS__)


// dumps stack in case of a buggy condition
#define CONGFS_BUG_ON(condition, msgStr) \
   do { \
...
#define CONGFS_BUG_ON_DEBUG(condition, msgStr)   CONGFS_BUG_ON(condition, msgStr)
...
#define CONGFS_BUG_ON_DEBUG(condition, msgStr) \
   do { /* nothing */ } while(0)

#endif // LOG_DEBUG_MESSAGES

#ifdef CONGFS_OPENTK_LOG_CONN_ERRORS
...
#endif // CONGFS_OPENTK_LOG_CONN_ERRORS
...
/**
 * CONGFS_KFREE_LIST() - destroys and kfree()s all element of a list, leaving an empty list
 *
 * expands to ``CONGFS_KFREE_LIST_DTOR(List, ElemType, Member, (void))``, ie no destructor is
 * called.
 */
#define CONGFS_KFREE_LIST(List, ElemType, Member) \
   CONGFS_KFREE_LIST_DTOR(List, ElemType, Member, (void))
/**
 * CONGFS_KFREE_LIST_DTOR() - destroys and kfree()s all element of a list, leaving an empty list
 ...
 */
#define CONGFS_KFREE_LIST_DTOR(List, ElemType, Member, Dtor) \
   do { \
      ElemType* entry; \
      ElemType* n; \
 ...
 */
#define CONGFS_RB_KEYCMP_LT_INTEGRAL(lhs, rhs) \
   ( lhs < rhs ? -1 : (lhs == rhs ? 0 : 1) )

/**
 * CONGFS_KFREE_RBTREE() - destroys and kfree()s all elements an rbtree
 *
 * expands to ``CONGFS_KFREE_RBTREE_DTOR(TreeRoot, ElemType, Member, (void))``, ie no destructor is
 * called.
 */
#define CONGFS_KFREE_RBTREE(TreeRoot, ElemType, Member) \
   CONGFS_KFREE_RBTREE_DTOR(TreeRoot, ElemType, Member, (void))
/**
 * CONGFS_KFREE_RBTREE_DTOR() - destroys and kfree()s all elements an rbtree, leaving an empty tree
 ...
 */
#define CONGFS_KFREE_RBTREE_DTOR(TreeRoot, ElemType, Member, Dtor) \
   do { \
      ElemType* pos; \
      ElemType* n; \
...
/**
 * CONGFS_RBTREE_FUNCTIONS() - defines a default set of rbtree functions
 ...
 *    if necessary.
 */
#define CONGFS_RBTREE_FUNCTIONS(Access, NS, RootTy, RootNode, KeyTy, ElemTy, ElemKey, ElemNode, \
      KeyCmp) \
...
#define CONGFS_RBTREE_FOR_EACH_ENTRY(Pos, Root, Node) \
...
#define CONGFS_DATA_VERSION ((uint32_t)0)
