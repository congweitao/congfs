...
/**
 * Note: Uses the number of retries that has conn defined in the app config.
 *
 * @param outRespBuf must be returned to the store - not freed!
 */
   ...
   RequestResponseArgs* rrArgs)
{
   ...
   CONGFS_BUG_ON_DEBUG(rrNode->targetStates == NULL, "targetStates missing");
   CONGFS_BUG_ON_DEBUG(rrNode->mirrorBuddies == NULL, "mirrorBuddies missing");
   ...
   for( ; ; ) // retry loop
   {
      ...
      if(!rrArgs->node)
      {
         ...
         CONGFS_BUG_ON_DEBUG(Node_getNumID(rrArgs->node).value != nodeID.value,
