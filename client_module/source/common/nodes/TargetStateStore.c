...
CONGFS_RBTREE_FUNCTIONS(static, _TargetStateStore, struct TargetStateStore, states,
      uint16_t,
      struct TargetStateInfo, targetID, _node,
      CONGFS_RB_KEYCMP_LT_INTEGRAL)
...
{
   CONGFS_KFREE_RBTREE(&this->states, struct TargetStateInfo, _node);
}
   ...
   UInt8List* outReachabilityStates, UInt8List* outConsistencyStates)
{
   ...
   CONGFS_RBTREE_FOR_EACH_ENTRY(stateInfo, &this->states, _node)
   {
