...
{
   {CONGFS_RWTYPE_READ,  "read vec"},
   {CONGFS_RWTYPE_WRITE, "write vec"},
};

#define __FHGFSOPS_REMOTING_RW_SIZE \
   ( (sizeof(__Fhgfs_RWTypeList) ) / (sizeof(struct Fhgfs_RWTypeStrEntry) ) )

#define __FHGFSOPS_REMOTING_msgBufCacheName CONGFS_MODULE_NAME_STR "-pageVecMsgBufs"
...
bool FhgfsOpsRemoting_initMsgBufCache(void)
{
   FhgfsOpsRemoting_msgBufCache = OsCompat_initKmemCache(__FHGFSOPS_REMOTING_msgBufCacheName,
      CONGFS_COMMKIT_MSGBUF_SIZE, NULL);
...
/**
 * Check if there have conn errors during _writefile().
 *
   ...
   RemotingIOInfo* ioInfo)
{
   ...
   CONGFS_IOV_ITER_INIT(&iter, WRITE, &iov, 1, size);
   ...
   RemotingIOInfo* ioInfo, FhgfsInode* fhgfsInode)
{
   ...
   CONGFS_IOV_ITER_INIT(&iter, READ, &iov, 1, size);
   ...
   RemotingIOInfo* ioInfo, FhgfsInode* fhgfsInode)
{
   ...
   while(toBeRead)
   {
      ...
      list_for_each_entry(state, &stateList, base.base.targetInfoList)
      {
         ...
         { // we have an end of file here (but some data might have conn read)
