   ...
   FhgfsCommKitVec* comm)
{
   ...
   NetMessage_serialize( (NetMessage*)&readMsg, comm->msgBuf, CONGFS_COMMKIT_MSGBUF_SIZE);
...
#if (CONGFS_COMMKIT_DEBUG & COMMKIT_DEBUG_READ_SEND )
...
#if (CONGFS_COMMKIT_DEBUG & COMMKIT_DEBUG_READ_HEADER)
...
#if (CONGFS_COMMKIT_DEBUG & COMMKIT_DEBUG_RECV_DATA)
   if (recvRes > 0 && jiffies % CommKitErrorInjectRate == 0)
   {
      Logger_logTopFormatted(commHelper->log, LogTopic_COMMKIT, Log_WARNING, __func__,
         "Injecting timeout error after (successfully) receiving data.");
   ...
   FhgfsCommKitVec* comm)
{
   ...
   NetMessage_serialize( (NetMessage*)&writeMsg, comm->msgBuf, CONGFS_COMMKIT_MSGBUF_SIZE);
...
#if (CONGFS_COMMKIT_DEBUG & COMMKIT_DEBUG_WRITE_HEADER )
...
#if (CONGFS_COMMKIT_DEBUG & COMMKIT_DEBUG_WRITE_SEND)
   ...
   FhgfsCommKitVec* comm)
{
   ...
   respRes = MessagingTk_recvMsgBuf(commHelper->app, comm->sock,
      comm->msgBuf, CONGFS_COMMKIT_MSGBUF_SIZE);

#if (CONGFS_COMMKIT_DEBUG & COMMKIT_DEBUG_WRITE_RECV)
...
static int64_t __FhgfsOpsCommKitVec_writefileCommunicate(CommKitVecHelper* commHelper,
   FhgfsCommKitVec* comm)
{
   FhgfsInode* inode = CONGFS_INODE(comm->pageVec->inode);
   ...
   FhgfsCommKitVec* comm, Fhgfs_RWType rwType)
{
   ...
   CommKitVecHelper commHelper =
   {
      ...
      .logContext = (rwType == CONGFS_RWTYPE_READ) ?
