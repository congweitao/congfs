...
{
   const char* cacheName = CONGFS_MODULE_NAME_STR "-msgheaders";

   if(CONGFS_COMMKIT_MSGBUF_SIZE > PAGE_SIZE)
   {
      WARN_ON(1);
      return false;
   }
   ...
   headerBufferCache = kmem_cache_create(cacheName, CONGFS_COMMKIT_MSGBUF_SIZE, 0,
   ...
   int events)
{
   ...
   if(unlikely(context->pollTimedOut) || CONGFS_SHOULD_FAIL(commkit_polltimeout, 1) )
   {
   ...
   struct CommKitTargetInfo* info)
{
   ...
   if(CONGFS_SHOULD_FAIL(commkit_sendheader_timeout, 1) )
      sendRes = -ETIMEDOUT;
   else
...
static void __commkit_senddata_generic(CommKitContext* context, struct CommKitTargetInfo* info)
{
   ...
   if(sendRes == 0)
   { // all of the data has conn sent => proceed to the next stage
...
static void __commkit_recvheader_generic(CommKitContext* context, struct CommKitTargetInfo* info)
{
   ...
   if(CONGFS_SHOULD_FAIL(commkit_recvheader_timeout, 1) )
      recvRes = -ETIMEDOUT;
   else
   {
      size_t msgLength;
      ...
      if(msgLength > CONGFS_COMMKIT_MSGBUF_SIZE)
      { // message too big to be accepted
         Logger_logFormatted(context->log, Log_WARNING, context->ops->logContext,
            "Received a message that is too large from: %s (bufLen: %u, msgLen: %zdd)",
            Socket_getPeername(info->socket), CONGFS_COMMKIT_MSGBUF_SIZE, msgLength);
   ...
   struct CommKitTargetInfo* info)
{
   ...
   NetMessage_serialize(&readMsg.netMessage, info->headerBuffer, CONGFS_COMMKIT_MSGBUF_SIZE);
   ...
   void* buffer, size_t length, bool exact)
{
   ...
   if(CONGFS_SHOULD_FAIL(commkit_readfile_receive_timeout, 1) )
      recvRes = -ETIMEDOUT;
   else
   if(exact)
...
static int __commkit_readfile_recvdata(CommKitContext* context, struct CommKitTargetInfo* info)
{
   ...
   { // all of the data has conn received => receive the next lengthInfo in the header stage
      currentState->receiveFileData = false;
   }
   ...
   struct CommKitTargetInfo* info)
{
   ...
   NetMessage_serialize(&writeMsg.netMessage, info->headerBuffer, CONGFS_COMMKIT_MSGBUF_SIZE);
   ...
   struct CommKitTargetInfo* info)
{
   ...
   if(CONGFS_SHOULD_FAIL(commkit_writefile_senddata_timeout, 1) )
   {
      sendRes = -ETIMEDOUT;
      goto sendsDone;
   }
   ...
   struct CommKitTargetInfo* info)
{
   ...
   NetMessage_serialize(&requestMsg.netMessage, info->headerBuffer, CONGFS_COMMKIT_MSGBUF_SIZE);
