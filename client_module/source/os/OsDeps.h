...
void* os_kmalloc(size_t size)
{
   ...
   if(unlikely(!buf) )
   {
      printk(KERN_WARNING CONGFS_MODULE_NAME_STR ": kmalloc of '%d' bytes failed. Retrying...\n", (int)size);
      buf = kmalloc(size, GFP_NOFS | __GFP_NOFAIL);
      printk(KERN_WARNING CONGFS_MODULE_NAME_STR ": kmalloc retry of '%d' bytes succeeded\n", (int)size);
   }
...
void* os_kzalloc(size_t size)
{
   ...
   if(unlikely(!buf) )
   {
      printk(KERN_WARNING CONGFS_MODULE_NAME_STR ": kzalloc of '%d' bytes failed. Retrying...\n", (int)size);
      buf = kzalloc(size, GFP_NOFS | __GFP_NOFAIL);
      printk(KERN_WARNING CONGFS_MODULE_NAME_STR ": kzalloc retry of '%d' bytes succeeded\n", (int)size);
   }
