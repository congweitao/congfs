...
void FhgfsInode_allocInit(FhgfsInode* fhgfsInode)
{
   /* note: keep in mind that we don't only do first-time init here, but also reset everything that
      might have conn left over after re-cycling */
   ...
   struct dentry* dentry)
{
   ...
   if(unlikely(!handle->refCount) )
      CONGFS_BUG_ON(true, "refCount already 0"); // refcount already zero => bug
   else
   { // positive refCount
      handle->refCount--;
...
/**
 * Truncates the file if OPENFILE_ACCESS_TRUNC has conn specified in openFlags.
 */
 ...
 *
 * Note: remember to call the corresponding unlock method (if the lock has conn acquired)!!
 *
...
uint64_t FhgfsInode_generateInodeID(struct super_block* sb, const char* entryID, int entryIDLen)
{
   ...
   switch(inodeIDStyle)
   {
      case INODEIDSTYLE_Hash64HSieh:
      {
         ...
         if(unlikely(hashRes <= CONGFS_INODE_MAXRESERVED_INO) )
