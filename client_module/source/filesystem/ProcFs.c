...
#define CONGFS_PROC_DIR_NAME      "fs/" CONGFS_MODULE_NAME_STR
#define CONGFS_PROC_NAMEBUF_LEN   4096

#define CONGFS_PROC_ENTRY_CONFIG              "config"
#define CONGFS_PROC_ENTRY_STATUS              ".status"
#define CONGFS_PROC_ENTRY_MGMTNODES           "mgmt_nodes"
#define CONGFS_PROC_ENTRY_METANODES           "meta_nodes"
#define CONGFS_PROC_ENTRY_STORAGENODES        "storage_nodes"
#define CONGFS_PROC_ENTRY_CLIENTINFO          "client_info"
#define CONGFS_PROC_ENTRY_RETRIESENABLED      "conn_retries_enabled"
#define CONGFS_PROC_ENTRY_NETBENCHENABLED     "netbench_mode"
#define CONGFS_PROC_ENTRY_DROPCONNS           "drop_conns"
#define CONGFS_PROC_ENTRY_LOGLEVELS           "log_levels"
#define CONGFS_PROC_ENTRY_METATARGETSTATES    "meta_target_state"
#define CONGFS_PROC_ENTRY_STORAGETARGETSTATES "storage_target_state"


/**
 * Initializer for read-only proc file ops
 */
#define CONGFS_PROC_FOPS_INITIALIZER  \
   .open    = __ProcFs_open,         \
   .read    = seq_read,              \
   .llseek  = seq_lseek,             \
...
{
   { CONGFS_PROC_ENTRY_CONFIG, &__ProcFs_readV2_config },
   { CONGFS_PROC_ENTRY_STATUS, &__ProcFs_readV2_status },
   { CONGFS_PROC_ENTRY_MGMTNODES, &__ProcFs_readV2_mgmtNodes },
   { CONGFS_PROC_ENTRY_METANODES, &__ProcFs_readV2_metaNodes },
   { CONGFS_PROC_ENTRY_STORAGENODES, &__ProcFs_readV2_storageNodes },
   { CONGFS_PROC_ENTRY_CLIENTINFO, &__ProcFs_readV2_clientInfo },
   { CONGFS_PROC_ENTRY_METATARGETSTATES, &__ProcFs_readV2_metaTargetStates },
   { CONGFS_PROC_ENTRY_STORAGETARGETSTATES, &__ProcFs_readV2_storageTargetStates },
...
{
   { CONGFS_PROC_ENTRY_RETRIESENABLED, &__ProcFs_readV2_connRetriesEnabled,
      {
         CONGFS_PROC_FOPS_INITIALIZER,
         .write   = &__ProcFs_writeV2_connRetriesEnabled,
      },
   },
   { CONGFS_PROC_ENTRY_NETBENCHENABLED, &__ProcFs_readV2_netBenchModeEnabled,
      {
         CONGFS_PROC_FOPS_INITIALIZER,
         .write   = &__ProcFs_writeV2_netBenchModeEnabled,
      },
   },
   { CONGFS_PROC_ENTRY_DROPCONNS, &__ProcFs_readV2_nothing,
      {
         CONGFS_PROC_FOPS_INITIALIZER,
         .write   = &__ProcFs_writeV2_dropConns,
      },
   },
   { CONGFS_PROC_ENTRY_LOGLEVELS, &__ProcFs_readV2_logLevels,
      {
         CONGFS_PROC_FOPS_INITIALIZER,
...
void ProcFs_createGeneralDir(void)
{
   ...
   procDir = proc_mkdir(CONGFS_PROC_DIR_NAME, NULL);
   if(!procDir)
      printk_fhgfs(KERN_INFO, "Failed to create proc dir: " CONGFS_PROC_DIR_NAME "\n");
}
...
{
   remove_proc_entry(CONGFS_PROC_DIR_NAME, NULL);
}
...
void ProcFs_createEntries(App* app)
{
   ...
   char* dirNameBuf = vmalloc(CONGFS_PROC_NAMEBUF_LEN);

   scnprintf(dirNameBuf, CONGFS_PROC_NAMEBUF_LEN, CONGFS_PROC_DIR_NAME "/%s", sessionID);
...
{
   char* dirNameBuf = vmalloc(CONGFS_PROC_NAMEBUF_LEN);
   char* entryNameBuf = vmalloc(CONGFS_PROC_NAMEBUF_LEN);
   ...
   // remove entries
   scnprintf(entryNameBuf, CONGFS_PROC_NAMEBUF_LEN, "%s/%s",
      dirNameBuf, CONGFS_PROC_ENTRY_CONFIG);
   remove_proc_entry(entryNameBuf, NULL);

   scnprintf(entryNameBuf, CONGFS_PROC_NAMEBUF_LEN, "%s/%s",
      dirNameBuf, CONGFS_PROC_ENTRY_STATUS);
   remove_proc_entry(entryNameBuf, NULL);

   scnprintf(entryNameBuf, CONGFS_PROC_NAMEBUF_LEN, "%s/%s",
      dirNameBuf, CONGFS_PROC_ENTRY_MGMTNODES);
   remove_proc_entry(entryNameBuf, NULL);

   scnprintf(entryNameBuf, CONGFS_PROC_NAMEBUF_LEN, "%s/%s",
      dirNameBuf, CONGFS_PROC_ENTRY_METANODES);
   remove_proc_entry(entryNameBuf, NULL);

   scnprintf(entryNameBuf, CONGFS_PROC_NAMEBUF_LEN, "%s/%s",
      dirNameBuf, CONGFS_PROC_ENTRY_STORAGENODES);
   remove_proc_entry(entryNameBuf, NULL);

   scnprintf(entryNameBuf, CONGFS_PROC_NAMEBUF_LEN, "%s/%s",
      dirNameBuf, CONGFS_PROC_ENTRY_CLIENTINFO);
   remove_proc_entry(entryNameBuf, NULL);

   scnprintf(entryNameBuf, CONGFS_PROC_NAMEBUF_LEN, "%s/%s",
      dirNameBuf, CONGFS_PROC_ENTRY_METATARGETSTATES);
   remove_proc_entry(entryNameBuf, NULL);

   scnprintf(entryNameBuf, CONGFS_PROC_NAMEBUF_LEN, "%s/%s",
      dirNameBuf, CONGFS_PROC_ENTRY_STORAGETARGETSTATES);
   remove_proc_entry(entryNameBuf, NULL);

   scnprintf(entryNameBuf, CONGFS_PROC_NAMEBUF_LEN, "%s/%s",
      dirNameBuf, CONGFS_PROC_ENTRY_RETRIESENABLED);
   remove_proc_entry(entryNameBuf, NULL);

   scnprintf(entryNameBuf, CONGFS_PROC_NAMEBUF_LEN, "%s/%s",
      dirNameBuf, CONGFS_PROC_ENTRY_NETBENCHENABLED);
   remove_proc_entry(entryNameBuf, NULL);

   scnprintf(entryNameBuf, CONGFS_PROC_NAMEBUF_LEN, "%s/%s",
      dirNameBuf, CONGFS_PROC_ENTRY_DROPCONNS);
   remove_proc_entry(entryNameBuf, NULL);

   scnprintf(entryNameBuf, CONGFS_PROC_NAMEBUF_LEN, "%s/%s",
      dirNameBuf, CONGFS_PROC_ENTRY_LOGLEVELS);
   remove_proc_entry(entryNameBuf, NULL);
