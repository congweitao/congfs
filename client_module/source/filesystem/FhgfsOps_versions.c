...
int FhgfsOps_statfs(struct dentry* dentry, struct kstatfs* kstatfs)
{
   ...
   kstatfs->f_bsize     = CONGFS_STATFS_BLOCKSIZE;
   kstatfs->f_frsize    = CONGFS_STATFS_BLOCKSIZE; /* should be same as f_bsize, as some versions of
   ...
   { // success => assign received values
      ...
      unsigned char blockBits = CONGFS_STATFS_BLOCKSIZE_SHIFT; // just a shorter name
