...
/* read-ahead size is limited by CONGFS_DEFAULT_READAHEAD_PAGES, so this is the maximum already going
...
#define CONGFS_DEFAULT_READAHEAD_PAGES CONGFS_MAX_PAGE_LIST_SIZE

static struct file_system_type fhgfs_fs_type =
{
   .name       = CONGFS_MODULE_NAME_STR,
   .owner      = THIS_MODULE,
...
void __FhgfsOps_uninitApp(App* app)
{
   ...
   /* note: some of the procfs entries (e.g. remove_node) won't work anymore after app components
      have conn stopped, but others are still useful for finding reasons why app stop is delayed
...
int __FhgfsOps_constructFsInfo(struct super_block* sb, void* rawMountOptions)
{
   ...
   #if defined(KERNEL_HAS_SUPER_SETUP_BDI_NAME) && !defined(KERNEL_HAS_BDI_SETUP_AND_REGISTER)
   {
      ...
      res = super_setup_bdi_name(sb, CONGFS_MODULE_NAME_STR "-%ld",
       ...
       */
      bdi->ra_pages = CONGFS_DEFAULT_READAHEAD_PAGES;

      #if defined(KERNEL_HAS_BDI_CAP_MAP_COPY) 
         res = bdi_setup_and_register(bdi, CONGFS_MODULE_NAME_STR, BDI_CAP_MAP_COPY);
      #else
         res = bdi_setup_and_register(bdi, CONGFS_MODULE_NAME_STR);
      #endif
   #endif
   ...
   printk_fhgfs(KERN_INFO, "ConGFS mount ready.\n");
...
//call destroy iff not initialised/registered by super_setup_bdi_name
      ...
      printk_fhgfs(KERN_INFO, "ConGFS unmounted.\n");
   }
}
...
int FhgfsOps_fillSuper(struct super_block* sb, void* rawMountOptions, int silent)
{
   ...
   kstat.ino = CONGFS_INODE_ROOT_INO;
