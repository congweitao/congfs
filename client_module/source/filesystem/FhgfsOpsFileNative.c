...
static ssize_t __congfs_direct_IO(int rw, struct kiocb* iocb, struct iov_iter* iter, loff_t offset);

static struct workqueue_struct* remoting_io_queue;

bool congfs_native_init()
{
   ...
   remoting_io_queue = create_workqueue("congfs/flush");
   ...
   remoting_io_queue = alloc_workqueue("congfs/flush", WQ_RESCUER, num_online_cpus());
#else
   remoting_io_queue = alloc_workqueue("congfs/flush", WQ_MEM_RECLAIM, num_online_cpus());
#endif
...
static void congfs_drop_all_caches(struct inode* inode)
{
   os_inode_lock(inode);
...
static int congfs_release_range(struct file* filp, loff_t first, loff_t last)
{
   int writeRes;
...
static int congfs_acquire_range(struct file* filp, loff_t first, loff_t last)
{
   App* app = FhgfsOps_getApp(file_dentry(filp)->d_sb);
   ...
   err = congfs_release_range(filp, first, last);
   if(err)
      return err;
...
static int congfs_open(struct inode* inode, struct file* filp)
{
   App* app = FhgfsOps_getApp(file_dentry(filp)->d_sb);
   struct FhgfsInode* fhgfsInode = CONGFS_INODE(inode);
   int err;
   ...
   if (mustAcquire)
      err = congfs_acquire_range(filp, 0, -1);

   return err;
}

static int congfs_flush(struct file* filp, fl_owner_t id)
{
   FhgfsInode* inode = CONGFS_INODE(file_inode(filp));
   ...
   if(filp->f_flags & O_APPEND)
      AtomicInt_dec(&CONGFS_INODE(file_inode(filp) )->appendFDsOpen);
    ...
    * here. clearing the flag only after everything has conn written out requires exclusion
    * between modifiers and flushers, which is prohibitively expensive. */
   atomic_set(&inode->modified, 0);

   result = congfs_release_range(filp, 0, -1);
   if (result < 0)
   {
...
static int congfs_release(struct inode* inode, struct file* filp)
{
   int flushRes;
   ...
   flushRes = congfs_release_range(filp, 0, -1);

   if(flushRes < 0)
      congfs_drop_all_caches(inode);

   return FhgfsOps_release(inode, filp);
}

#if defined(KERNEL_HAS_AIO_WRITE_BUF)
static ssize_t congfs_file_write_iter(struct kiocb* iocb, struct iov_iter* from)
{
...
static struct append_range_descriptor* congfs_get_or_create_ard(struct address_space* mapping,
...
static ssize_t congfs_append_iter(struct kiocb* iocb, struct iov_iter* from)
{
   struct file* filp = iocb->ki_filp;
   struct FhgfsInode* inode = CONGFS_INODE(filp->f_mapping->host);
   ...
   do {
       ...
       * file. */
      ard = congfs_get_or_create_ard(inode->vfs_inode.i_mapping, iocb->ki_pos, size);
      if(IS_ERR(ard) )
      {
         written = PTR_ERR(ard);
         break;
      ...
      written = congfs_file_write_iter(iocb, from);
...
static ssize_t congfs_write_iter(struct kiocb* iocb, struct iov_iter* from)
{
   ...
   atomic_set(&CONGFS_INODE(file_inode(filp))->modified, 1);

   if(filp->f_flags & O_APPEND)
      return congfs_append_iter(iocb, from);

   if (!is_pipe_iter(from)
         && (from->count >= Config_getTuneFileCacheBufSize(App_getConfig(app))
               || CONGFS_SHOULD_FAIL(write_force_cache_bypass, 1)))
   {
      ssize_t result;
   ...
   return congfs_file_write_iter(iocb, from);
}

#if !defined(KERNEL_HAS_WRITE_ITER)
static ssize_t congfs_aio_write_iov(struct kiocb* iocb, const struct iovec* iov,
   unsigned long nr_segs, loff_t pos)
{
   struct iov_iter iter;
   size_t count = iov_length(iov, nr_segs);

   CONGFS_IOV_ITER_INIT(&iter, WRITE, iov, nr_segs, count);

   return congfs_write_iter(iocb, &iter);
}

#if defined(KERNEL_HAS_AIO_WRITE_BUF)
static ssize_t congfs_aio_write(struct kiocb* iocb, const char __user* buf, size_t count,
   loff_t pos)
{
   struct iovec iov = {
      .iov_base = (char*) buf,
   ...
   return congfs_aio_write_iov(iocb, &iov, 1, pos);
}
#else
static ssize_t congfs_aio_write(struct kiocb* iocb, const struct iovec* iov,
   unsigned long nr_segs, loff_t pos)
{
   return congfs_aio_write_iov(iocb, iov, nr_segs, pos);
}
#endif
#endif
...
static ssize_t congfs_file_read_iter(struct kiocb* iocb, struct iov_iter* from)
{
   struct iovec iov = iov_iter_iovec(from);
...
static ssize_t congfs_read_iter(struct kiocb* iocb, struct iov_iter* to)
{
   struct file* filp = iocb->ki_filp;
   ...
   if (!is_pipe_iter(to)
         && (to->count >= Config_getTuneFileCacheBufSize(App_getConfig(app))
               || CONGFS_SHOULD_FAIL(read_force_cache_bypass, 1)))
   {
      ...
      result = congfs_release_range(filp, iocb->ki_pos, iocb->ki_pos + count - 1);
      if(!result)
      {
         struct iov_iter data = *to;
         result = __congfs_direct_IO(READ, iocb, &data, iocb->ki_pos);
      }
   ...
   return congfs_file_read_iter(iocb, to);
}

#ifndef KERNEL_HAS_WRITE_ITER
static ssize_t congfs_aio_read_iov(struct kiocb* iocb, const struct iovec* iov,
   unsigned long nr_segs, loff_t pos)
{
   struct iov_iter iter;
   size_t count = iov_length(iov, nr_segs);

   CONGFS_IOV_ITER_INIT(&iter, READ, iov, nr_segs, count);

   return congfs_read_iter(iocb, &iter);
}

#ifdef KERNEL_HAS_AIO_WRITE_BUF
static ssize_t congfs_aio_read(struct kiocb* iocb, char __user* buf, size_t count,
   loff_t pos)
{
   struct iovec iov = {
      .iov_base = buf,
   ...
   return congfs_aio_read_iov(iocb, &iov, 1, pos);
}
#else
static ssize_t congfs_aio_read(struct kiocb* iocb, const struct iovec* iov,
   unsigned long nr_segs, loff_t pos)
{
   return congfs_aio_read_iov(iocb, iov, nr_segs, pos);
}
#endif
#endif


static int __congfs_fsync(struct file* filp, loff_t start, loff_t end, int datasync)
{
   App* app = FhgfsOps_getApp(file_dentry(filp)->d_sb);
   Config* cfg = App_getConfig(app);
   int err;
   FhgfsInode* fhgfsInode = CONGFS_INODE(file_inode(filp));
   ...
   /* see comment in congfs_flush for explanation */
   atomic_set(&fhgfsInode->modified, 0);

   err = congfs_release_range(filp, start, end);
   if(err)
   {
      atomic_set(&fhgfsInode->modified, 0);
      return err;
...
static int congfs_fsync(struct file* file, loff_t start, loff_t end, int datasync)
{
   return __congfs_fsync(file, start, end, datasync);
}
#elif defined(KERNEL_HAS_FSYNC_2)
static int congfs_fsync(struct file* file, int datasync)
{
   return __congfs_fsync(file, 0, LLONG_MAX, datasync);
}
#else
static int congfs_fsync(struct file* file, struct dentry* dentry, int datasync)
{
   return __congfs_fsync(file, 0, LLONG_MAX, datasync);
}
#endif

static int congfs_flock(struct file* filp, int cmd, struct file_lock* flock)
{
   App* app = FhgfsOps_getApp(file_dentry(filp)->d_sb);
   int err = -EINVAL;
   ...
   switch(flock->fl_type)
   {
      ...
      case F_UNLCK:
         err = congfs_release_range(filp, 0, -1);
         break;
   }
...
static int congfs_lock(struct file* filp, int cmd, struct file_lock* flock)
{
   App* app = FhgfsOps_getApp(file_dentry(filp)->d_sb);
   int err = -EINVAL;
   ...
   switch(flock->fl_type)
   {
      case F_RDLCK:
      case F_WRLCK:
         err = congfs_acquire_range(filp, flock->fl_start, flock->fl_end);
         break;

      case F_UNLCK:
         err = congfs_release_range(filp, flock->fl_start, flock->fl_end);
         break;
   }
...
static int congfs_mmap(struct file* filp, struct vm_area_struct* vma)
{
   App* app = FhgfsOps_getApp(file_dentry(filp)->d_sb);
   int err = -EINVAL;
...
const struct file_operations fhgfs_file_native_ops = {
   ...
   .read = do_sync_read,
   .write = do_sync_write,
   .aio_read = congfs_aio_read,
   .aio_write = congfs_aio_write,
#else
   .read_iter = congfs_read_iter,
   .write_iter = congfs_write_iter,
#endif
...
struct congfs_writepages_context
{
   RemotingIOInfo ioInfo;
   ...
   struct congfs_writepages_state* currentState;
   int submitted;
...
struct congfs_writepages_state
{
   struct page** pages;
   struct iovec* iov;
   unsigned nr_pages;

   struct congfs_writepages_context* context;
...
{
   struct congfs_writepages_state* state;
...
{
   struct congfs_writepages_state* state = element;
...
static struct congfs_writepages_state* wps_alloc(struct congfs_writepages_context* ctx)
{
   struct congfs_writepages_state* result = mempool_alloc(writepages_pool, GFP_NOFS);
...
static void wps_free(struct congfs_writepages_state* state)
{
   mempool_free(state, writepages_pool);
}

static int congfs_allocate_ard(struct congfs_writepages_state* state)
{
   ...
   if(!Config_getTuneUseGlobalAppendLocks(App_getConfig(app) ) )
   {
      ...
      appendRes = FhgfsOpsHelper_appendfileVecOffset(CONGFS_INODE(mapping->host), &reserveIov, 1,
   ...
   if(!state->context->appendLockTaken)
   {
      ...
      FhgfsOpsErr statRes;
      fhgfs_stat stat;
      FhgfsInode* inode = CONGFS_INODE(mapping->host);
...
static int congfs_wps_prepare(struct congfs_writepages_state* state, loff_t* offset, size_t* size)
{
   struct append_range_descriptor* ard;
   int err;
   int i;
...
static int congfs_writepages_work(struct congfs_writepages_state* state)
{
   App* app;
   struct iov_iter iter;
   unsigned i;
   ...
   result = congfs_wps_prepare(state, &offset, &size);
   if(result < 0)
      goto writes_done;

   if(CONGFS_SHOULD_FAIL(writepage, 1) )
   {
      result = -EIO;
      goto artifical_write_error;
   }

   CONGFS_IOV_ITER_INIT(&iter, WRITE, state->iov, state->nr_pages, size);
...
static void congfs_writepages_work_wrapper(struct work_struct* w)
{
   struct congfs_writepages_state* state = container_of(w, struct congfs_writepages_state, work);
   SynchronizedCounter* barrier = &state->context->barrier;

   congfs_writepages_work(state);
   SynchronizedCounter_incCount(barrier);
}

static void congfs_writepages_submit(struct congfs_writepages_context* context)
{
   struct congfs_writepages_state* state = context->currentState;
   ...
   INIT_WORK(&state->work, congfs_writepages_work_wrapper);
   queue_work(remoting_io_queue, &state->work);
}

static bool congfs_wps_must_flush_before(struct congfs_writepages_state* state, struct page* next)
{
   struct append_range_descriptor* ard;
...
static int congfs_writepages_callback(struct page* page, struct writeback_control* wbc, void* data)
{
   struct congfs_writepages_context* context = data;
   struct congfs_writepages_state* state = context->currentState;

   BUG_ON(!pvr_present(page) && !ard_from_page(page) );

   if(congfs_wps_must_flush_before(state, page) )
   {
      congfs_writepages_submit(context);
...
static int congfs_do_write_pages(struct address_space* mapping, struct writeback_control* wbc,
   struct page* page, bool unlockPages)
{
   ...
   referenceRes = FhgfsInode_referenceHandle(CONGFS_INODE(inode), NULL, OPENFILE_ACCESS_READWRITE,
   ...
   FhgfsInode_getRefIOInfo(CONGFS_INODE(inode), handleType, OPENFILE_ACCESS_READWRITE,
      &context.ioInfo);

   FhgfsInode_incWriteBackCounter(CONGFS_INODE(inode) );
   ...
   if(Config_getTuneUseGlobalAppendLocks(App_getConfig(app) ) )
   {
      ...
      if( (page && ard_from_page(page) ) ||
          (!page && AtomicInt_read(&CONGFS_INODE(inode)->appendFDsOpen) ) )
      {
         wbc->range_cyclic = 0;
         wbc->range_start = 0;
   ...
   if(page)
      err = congfs_writepages_callback(page, wbc, &context);
   else
      err = write_cache_pages(mapping, wbc, congfs_writepages_callback, &context);
   ...
   if(context.appendLockTaken)
      FhgfsOpsHelper_releaseAppendLock(CONGFS_INODE(inode), &context.ioInfo);

   FhgfsInode_releaseHandle(CONGFS_INODE(inode), handleType, NULL);

   FhgfsInode_decWriteBackCounter(CONGFS_INODE(inode) );
   FhgfsInode_unsetNoIsizeDecrease(CONGFS_INODE(inode) );

   return err;
}

static int congfs_writepage(struct page* page, struct writeback_control* wbc)
{
   ...
   return congfs_do_write_pages(page->mapping, wbc, page, true);
}

static int congfs_writepages(struct address_space* mapping, struct writeback_control* wbc)
{
   struct inode* inode = mapping->host;
   App* app = FhgfsOps_getApp(inode->i_sb);
   ...
   return congfs_do_write_pages(mapping, wbc, NULL, true);
}



static int congfs_flush_page(struct page* page)
{
   struct writeback_control wbc = {
      .nr_to_write = 1,
   ...
   return congfs_do_write_pages(page->mapping, &wbc, page, false);
}

static int congfs_readpage(struct file* filp, struct page* page)
{
   App* app = FhgfsOps_getApp(file_dentry(filp)->d_sb);
   ...
   {
      readRes = congfs_flush_page(page);
      if(readRes)
         goto out;
   }

   if(CONGFS_SHOULD_FAIL(readpage, 1) )
      goto out;
...
struct congfs_readpages_context
{
   RemotingIOInfo ioInfo;
   FileHandleType handleType;
   struct inode* inode;

   struct congfs_readpages_state* currentState;

   struct kref refs;
};

static struct congfs_readpages_context* rpc_create(struct inode* inode)
{
   struct congfs_readpages_context* context;
   FhgfsOpsErr referenceRes;
   ...
   FhgfsInode_getRefIOInfo(CONGFS_INODE(inode), context->handleType, OPENFILE_ACCESS_READWRITE,
      &context->ioInfo);
...
static void __congfs_readpages_context_free(struct kref* ref)
{
   struct congfs_readpages_context* context;

   context = container_of(ref, struct congfs_readpages_context, refs);

   FhgfsInode_releaseHandle(CONGFS_INODE(context->inode), context->handleType, NULL);
   kfree(context);
}

static void rpc_get(struct congfs_readpages_context* context)
{
   kref_get(&context->refs);
}

static void rpc_put(struct congfs_readpages_context* context)
{
   kref_put(&context->refs, __congfs_readpages_context_free);
}


struct congfs_readpages_state
{
   struct page** pages;
   struct iovec* iov;
   unsigned nr_pages;
...
static struct congfs_readpages_state* rps_alloc(struct congfs_readpages_context* context)
{
   struct congfs_readpages_state* state;
...
static void rps_free(struct congfs_readpages_state* state)
{
   if(!state)
      return;
...
static void congfs_readpages_work(struct work_struct* w)
{
   struct congfs_readpages_state* state = container_of(w, struct congfs_readpages_state, work);
   ...
   CONGFS_IOV_ITER_INIT(&iter, READ, state->iov, state->nr_pages,
      state->nr_pages * PAGE_SIZE);

   readRes = FhgfsOpsRemoting_readfileVec(&iter, page_offset(state->pages[0]),
      &state->context->ioInfo, CONGFS_INODE(state->pages[0]->mapping->host) );
   if(readRes < 0)
...
static void congfs_readpages_submit(struct congfs_readpages_context* context)
{
   struct congfs_readpages_state* state = context->currentState;

   INIT_WORK(&state->work, congfs_readpages_work);
   queue_work(remoting_io_queue, &state->work);
}

static int congfs_readpages_add_page(void* data, struct page* page)
{
   struct congfs_readpages_context* context = data;
   struct congfs_readpages_state* state = context->currentState;
   bool mustFlush;
   ...
   if(mustFlush)
   {
      congfs_readpages_submit(context);
...
static int congfs_readpages(struct file* filp, struct address_space* mapping,
   struct list_head* pages, unsigned nr_pages)
{
   ...
   err = read_cache_pages(mapping, pages, congfs_readpages_add_page, context);
   congfs_readpages_submit(context);
...
static int __congfs_write_begin(struct file* filp, loff_t pos, unsigned len, struct page* page)
{
   struct append_range_descriptor* ard;
   int result = 0;
...
static int __congfs_write_end(struct file* filp, loff_t pos, unsigned len, unsigned copied,
   struct page* page)
{
   struct inode* inode = page->mapping->host;
   int result = copied;
   ...
   if(i_size_read(inode) < pos + copied)
   {
      ...
      FhgfsInode_setPageWriteFlag(CONGFS_INODE(inode) );
      FhgfsInode_setLastWriteBackOrIsizeWriteTime(CONGFS_INODE(inode) );
      FhgfsInode_setNoIsizeDecrease(CONGFS_INODE(inode) );
   }
...
static int congfs_prepare_write(struct file* filp, struct page* page, unsigned from, unsigned to)
{
   return __congfs_write_begin(filp, from, to - from + 1, page);
}

static int congfs_commit_write(struct file* filp, struct page* page, unsigned from, unsigned to)
{
   return __congfs_write_end(filp, from, to - from + 1, to - from + 1, page);
}
#else
static int congfs_write_begin(struct file* filp, struct address_space* mapping, loff_t pos,
   unsigned len, unsigned flags, struct page** pagep, void** fsdata)
{
   ...
   *pagep = grab_cache_page_write_begin(mapping, index, flags);
   ...
   return __congfs_write_begin(filp, pos, len, *pagep);
}

static int congfs_write_end(struct file* filp, struct address_space* mapping, loff_t pos,
   unsigned len, unsigned copied, struct page* page, void* fsdata)
{
   return __congfs_write_end(filp, pos, len, copied, page);
}
#endif

static int congfs_releasepage(struct page* page, gfp_t gfp)
{
...
static int congfs_set_page_dirty(struct page* page)
{
   atomic_set(&CONGFS_INODE(page->mapping->host)->modified, 1);

   if(ard_from_page(page) )
   {
      int err;

      err = congfs_flush_page(page);
      if(err < 0)
         return err;
   }
...
static void __congfs_invalidate_page(struct page* page, unsigned begin, unsigned end)
{
   if(pvr_present(page) )
   {
...
static void congfs_invalidate_page(struct page* page, unsigned long begin)
{
   __congfs_invalidate_page(page, begin, PAGE_CACHE_SIZE);
}
#else
static void congfs_invalidate_page(struct page* page, unsigned begin, unsigned end)
{
   __congfs_invalidate_page(page, begin, end);
}
#endif

static ssize_t congfs_dIO_read(struct kiocb* iocb, struct iov_iter* iter, loff_t offset,
   RemotingIOInfo* ioInfo)
{
   struct file* filp = iocb->ki_filp;
   struct inode* inode = file_inode(filp);
   ...
   congfs_iov_for_each(iov, loopIter, iter)
   {
...
static ssize_t congfs_dIO_write(struct kiocb* iocb, struct iov_iter* iter, loff_t offset,
   RemotingIOInfo* ioInfo)
{
   struct file* filp = iocb->ki_filp;
   struct inode* inode = file_inode(filp);
...
static ssize_t __congfs_direct_IO(int rw, struct kiocb* iocb, struct iov_iter* iter, loff_t offset)
{
   ...
   FsFileInfo_getIOInfo(__FhgfsOps_getFileInfo(filp), CONGFS_INODE(inode), &ioInfo);
   ...
   {
      case READ:
         return congfs_dIO_read(iocb, iter, offset, &ioInfo);

      case WRITE:
         return congfs_dIO_write(iocb, iter, offset, &ioInfo);
   }
...
static int congfs_launderpage(struct page* page)
{
   return congfs_flush_page(page);
}

const struct address_space_operations fhgfs_addrspace_native_ops = {
   .readpage = congfs_readpage,
   .writepage = congfs_writepage,
   .releasepage = congfs_releasepage,
   .set_page_dirty = congfs_set_page_dirty,
   .invalidatepage = congfs_invalidate_page,

   .readpages = congfs_readpages,
   .writepages = congfs_writepages,

   .launder_page = congfs_launderpage,

#ifdef KERNEL_HAS_PREPARE_WRITE
   .prepare_write = congfs_prepare_write,
   .commit_write = congfs_commit_write,
#else
   .write_begin = congfs_write_begin,
   .write_end = congfs_write_end,
#endif
};
