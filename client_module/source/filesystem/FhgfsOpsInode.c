...
#define FhgfsOpsInode_CACHE_NAME    CONGFS_MODULE_NAME_STR "_inode_cache"
   ...
   bool withFileSize, bool force)
{
   ...
   if(unlikely(!FhgfsOps_getIsRootInited(inode->i_sb) && inode->i_ino == CONGFS_INODE_ROOT_INO)
      || (!FhgfsInode_isCacheValid(CONGFS_INODE(inode), inode->i_mode, cfg) && whenCacheInvalid)
      || unlikely(force))
   {
      ...
      if (inode->i_ino == CONGFS_INODE_ROOT_INO)
...
#endif // KERNEL_HAS_ATOMIC_OPEN
{
   ...
   FhgfsInode* fhgfsParentInode = CONGFS_INODE(parentDir);
   ...
   /* check if root inode attribs have conn fetched already
...
int FhgfsOps_getattr(struct vfsmount* mnt, struct dentry* dentry, struct kstat* kstat)
{
   ...
   if(!retVal)
   {
      ...
      if(isRoot)
         kstat->ino = CONGFS_INODE_ROOT_INO; // root => assign the constant root inode number
      else
...
ssize_t FhgfsOps_listxattr(struct dentry* dentry, char* value, size_t size)
{
   ...
   FhgfsInode* fhgfsInode = CONGFS_INODE(dentry->d_inode);
...
#endif // KERNEL_HAS_UMODE_T
{
   ...
   int retVal = 0;
   FhgfsOpsErr mkRes;
   FhgfsInode* fhgfsParentInode = CONGFS_INODE(dir);
   EntryInfo newEntryInfo;
   const char* entryName = dentry->d_name.name;
 ...
 *
 * Warning: This is currently only enabled when define CONGFS_ENABLE_ATOMIC_OPEN is set, because
...
int FhgfsOps_unlink(struct inode* dir, struct dentry* dentry)
{
   ...
   { // remote success
      ...
      if(dentry->d_inode)
      {
         ...
         FhgfsInode* fhgfsInode = CONGFS_INODE(fileInode);
   ...
   struct dentry* toFileDentry)
{
   ...
   FhgfsInode* fhgfsFileInode = CONGFS_INODE(fileInode);

   FhgfsInode* fhgfsToDirInode    = CONGFS_INODE(toDirInode);

   struct inode* fromDirInode    = fromFileDentry->d_parent->d_inode;
   FhgfsInode* fhgfsFromDirInode  = CONGFS_INODE(fromDirInode);
...
static int __congfs_follow_link(struct dentry* dentry, char** linkBody, void** cookie)
{
   struct inode* inode = dentry->d_inode;
   App* app = FhgfsOps_getApp(inode->i_sb);
   Logger* log = App_getLogger(app);
   ...
   FhgfsInode* fhgfsParentInode = CONGFS_INODE(inode);
...
static void __congfs_put_link(void* cookie)
{
   ...
   struct delayed_call* done)
{
   ...
   if(!__congfs_follow_link(dentry, &destination, &cookie) )
      set_delayed_call(done, __congfs_put_link, cookie);
...
const char* FhgfsOps_follow_link(struct dentry* dentry, void** cookie)
{
   ...
   __congfs_follow_link(dentry, &destination, cookie);
   return destination;
}
   ...
   )
{
   ...
   FhgfsInode* fhgfsFromEntryInode = CONGFS_INODE(fromEntryInode);
   ...
   FhgfsInode* fhgfsFromDirInode = CONGFS_INODE(inodeDirFrom);
   FhgfsInode* fhgfsToDirInode   = CONGFS_INODE(inodeDirTo);
...
int __FhgfsOps_compareInodeID(struct inode* cachedInode, void* voidComparisonInfo)
{
   ...
   { // inode hashes match => compare entryID strings
      ...
      FhgfsInode* fhgfsInode = CONGFS_INODE(cachedInode);
