   ...
   int connectable)
{
   ...
   if (connectable)
   {
      FhgfsInode* fhgfsParentInode = CONGFS_INODE(parent_inode);
   ...
   struct FhgfsNfsFileHandleV3* fhgfsNfsHandle, bool lookupParent)
{
   ...
   if (strncmp(entryID, META_ROOTDIR_ID_STR, entryIDLen) == 0)
      inodeHash = CONGFS_INODE_ROOT_INO;
   else
   ...
   if (resDentry && !IS_ERR(resDentry) )
   {
      ...
      {
         FhgfsInode* fhgfsInode = CONGFS_INODE(resDentry->d_inode);
         const EntryInfo* entryInfo;
...
struct dentry* FhgfsOpsExport_getParentDentry(struct dentry* childDentry)
{
      ...
      FhgfsInode* fhgfsChildInode = CONGFS_INODE(childInode);
      ...
      if (strncmp(parentEntryID, META_ROOTDIR_ID_STR, parentIDLen) == 0)
         comparisonInfo.inodeHash = CONGFS_INODE_ROOT_INO; // root inode
      else
      ...
      { // not found in cache => try to get it from mds
         ...
         {  // update the parentEntryID
            FhgfsInode* parentFhgfsInode = CONGFS_INODE(parentInode);
   ...
   char* outName)
{
   ...
   bool retVal = false;
   struct inode* dirInode = dirDentry->d_inode;
   FhgfsInode* fhgfsDirInode = CONGFS_INODE(dirInode);
