...
#define CONGFS_INODE_ROOT_INO                 2 /* traditional root inode number */
#define CONGFS_INODE_MAXRESERVED_INO          CONGFS_INODE_ROOT_INO

#define CONGFS_INODE_FILEHANDLES_NUM          3 /* r, rw, w (see "enum FileHandleType") */

#define CONGFS_INODE(inodePointer)            ( (FhgfsInode*)inodePointer)
#define CONGFS_VFSINODE(fhgfsInodePointer)    ( (struct inode*)fhgfsInodePointer)


#define CONGFS_INODE_FLAG_WRITE_ERROR         1 // there was a write error with this inode
                                               // if set switches to sync writes to faster notify
                                               // applications
#define CONGFS_INODE_FLAG_PAGED               2 // the inode was written to from page functions
...
struct FhgfsInode
{
   ...
   FhgfsInodeFileHandle fileHandles[CONGFS_INODE_FILEHANDLES_NUM]; // use FileHandleType as index
   ...
   atomic_t modified; // tracks whether the inode data has conn written since its last full flush
};
...
bool FhgfsInode_getIsFileOpen(FhgfsInode* this)
{
   ...
   for(i=0; i < CONGFS_INODE_FILEHANDLES_NUM; i++)
   {
...
void _FhgfsInode_initRootEntryInfo(FhgfsInode* this)
{
   ...
   if (unlikely(!hasRootInfo) && (vfs_inode->i_ino == CONGFS_INODE_ROOT_INO) )
   {
...
{
   if(this->vfs_inode.i_ino == CONGFS_INODE_ROOT_INO)
...
void FhgfsInode_setPageWriteFlag(FhgfsInode* this)
{
   ...
   if (!FhgfsInode_getHasPageWriteFlag(this) )
      this->flags |= CONGFS_INODE_FLAG_PAGED;
}
...
{
   return this->flags & CONGFS_INODE_FLAG_PAGED;
}
...
{
   this->flags |= CONGFS_INODE_FLAG_WRITE_ERROR;
}
...
{
   return this->flags & CONGFS_INODE_FLAG_WRITE_ERROR;
}
