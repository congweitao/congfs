...
/**
 * Wrapper class for the calls to the congfs ioctl library.
 */
public class JConGFS {
   static {
      System.loadLibrary("jcongfs");
   }

   static final private int JCONGFS_API_MAJOR_VERSION = 1; // major version number of the API,
                                                           // different major version are 
                                                           // incompatible
   static final private int JCONGFS_API_MINOR_VERSION = 0; // minor version number of the API, the
   ...
   /**
    * Constructs a new JConGFS object for a file. The file is open()ed during construction of this
    ...
    */
   public JConGFS(String fileName) throws IOException, InterruptedIOException {
   ...
   public static boolean jBeegfsCheckApiVersion(int requiredMajorVersion, int requiredMinorVersion) {
      if(requiredMajorVersion != JCONGFS_API_MAJOR_VERSION)
         return false;

      if(requiredMinorVersion > JCONGFS_API_MINOR_VERSION)
         return false;
   ...
   /**
    * @return whether the location is on a ConGFS mount.
    */
   public native boolean isBeegFS();

   /**
    * @return the path to the ConGFS config file, null in case of an error.
    */
   public native String getConfigPath();

   /**
    * @return the path to the congfs-clien't's runtime config, null in case of an error.
    */
