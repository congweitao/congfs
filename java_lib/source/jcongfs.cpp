...
#include <congfs/congfs.h>

JNIEXPORT jint JNICALL Java_com_congfs_JConGFS_open(JNIEnv* env, jobject obj, jstring jFileName) {
...
/**
 * Helper function to access the member 'nativeFileDescriptor' in the Java 'JConGFS' object.
 */
int getFd(JNIEnv* env, jobject obj) {
   jclass jJConGFS_class = env->GetObjectClass(obj);
   jfieldID jNativeFileDescriptorFID = env->GetFieldID(jJConGFS_class, "nativeFileDescriptor", "I");

   jint jNativeFileDescriptor = env->GetIntField(obj, jNativeFileDescriptorFID);
   return static_cast<int>(jNativeFileDescriptor);
}

JNIEXPORT void JNICALL Java_com_congfs_JConGFS_close(JNIEnv* env, jobject obj) {
...
JNIEXPORT jboolean JNICALL Java_com_congfs_JConGFS_isBeegFS(JNIEnv* env, jobject obj) {
   return congfs_testIsConGFS(getFd(env, obj) );
}

JNIEXPORT jstring JNICALL Java_com_congfs_JConGFS_getConfigPath(JNIEnv* env, jobject obj) {
   char* configPath;
   bool res = congfs_getConfigFile(getFd(env, obj), &configPath);
   if (res) {
...
JNIEXPORT jstring JNICALL Java_com_congfs_JConGFS_getRuntimeConfigPath(JNIEnv* env, jobject obj) {
   char* runtimeConfigPath;
   bool res = congfs_getRuntimeConfigFile(getFd(env, obj), &runtimeConfigPath);
   if (res) {
...
JNIEXPORT jstring JNICALL Java_com_congfs_JConGFS_getMountID(JNIEnv* env, jobject obj) {
   char* mountID;
   bool res = congfs_getMountID(getFd(env, obj), &mountID);
   if (res) {
...
JNIEXPORT jobject JNICALL Java_com_congfs_JConGFS_getStripeInfo(JNIEnv* env, jobject obj) {
   unsigned patternType;
   unsigned chunkSize;
   uint16_t numTargets;
   if (congfs_getStripeInfo(getFd(env, obj), &patternType, &chunkSize, &numTargets) ) {
      jclass jstripeinfo = env->FindClass("com/congfs/JConGFS$StripeInfo");
...
JNIEXPORT jobject JNICALL Java_com_congfs_JConGFS_getStripeTarget(JNIEnv* env, jobject obj,
      jint jTargetIndex) {
   ...
   char* nodeName;
   if (congfs_getStripeTarget(fd, targetIndex, &targetNumID, &nodeNumID, &nodeName) ) {
      jclass jStripeTarget_class = env->FindClass("com/congfs/JConGFS$StripeTarget");
