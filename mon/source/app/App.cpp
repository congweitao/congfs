...
void App::run()
{
   ...
   catch (const InvalidConfigException& e)
   {
      ...
      err << "Config error: " << e.what() << std::endl
            << "[ConGFS Mon Version: " << CONGFS_VERSION << std::endl
            << "Refer to the default config file (/etc/congfs/congfs-mon.conf)" << std::endl
            << "or visit http://www.congfs.com to find out about configuration options.]";
...
void App::logInfos()
{
   LOG(GENERAL, CRITICAL, std::string("Version: ") + CONGFS_VERSION);
#ifdef CONGFS_DEBUG
