ConGFS monitoring service README
================================
...
The ConGFS monitoring service (congfs-mon) collects statistical data from the
various ConGFS nodes and stores it into a time series database (at the moment InfluxDB and Apache
...
* The user might want to run the InfluxDB server on another machine and/or wants
  to integrate the congfs-mon into an already existing setup.
...
So, to use congfs-mon, a working and reachable InfluxDB setup is required. Installing
InfluxDB should be simple in most cases since there are prebuilt packages available
for all of the distributions that are supported by ConGFS.
...
If your ConGFS setup, the congfs-mon daemon and InfluxDB are already running and are configured
...
To configure congfs-mon to use Cassandra, you need to install the datastax cassandra client library
on your system which you can find here: https://github.com/datastax/cpp-driver.
It has to be the version 2.9. congfs-mon loads the library dynamically, so no recompilation is
required. The congfs-mon config file needs to be edited to use the cassandra plugin. The available 
