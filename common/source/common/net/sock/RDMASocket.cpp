...
namespace {
   ...
   struct IBLibLoader {
      ...
      IBLibLoader() {
         ...
         iblib.reset(dlopen("libcongfs_ib.so", RTLD_NOW));
         // no ib support if lib can't be loaded. likely because ib libs are not
         // installed.
         if (!iblib)
            return;

         socket_impl = (RDMASocket::ImplCallbacks*) dlsym(iblib.get(), "congfs_socket_impl");
...
{
   if (!socket_impl)
      throw std::logic_error("congfs_rdma_socket_create called with no rdma support");
