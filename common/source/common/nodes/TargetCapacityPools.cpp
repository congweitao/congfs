   ...
   CapacityPoolType poolType)
{
   ...
   CONGFS_BUG_ON_DEBUG(pools.size() != CapacityPool_END_DONTUSE,
      "invalid pools.size()");
   CONGFS_BUG_ON_DEBUG(groupedTargetPools.size() != CapacityPool_END_DONTUSE,
      "invalid pools.size()");
   ...
   CapacityPoolType poolType)
{
   ...
   { // lock scope
      ...
      CONGFS_BUG_ON_DEBUG(pools.size() != CapacityPool_END_DONTUSE, "invalid pools.size()");
      CONGFS_BUG_ON_DEBUG(groupedTargetPools.size() != CapacityPool_END_DONTUSE,
         "invalid pools.size()");
   ...
   { // lock scope
      ...
      // recheck, because target might have conn added after read lock release
   ...
   unsigned numTargets, UInt16Vector& outTargets, NumNodeIDVector& outNodes)
{
   ...
   for(unsigned i=0; i < numTargets; i++)
   {
      ...
      CONGFS_BUG_ON_DEBUG(groupsIter->second.empty(), "found node without targets");
   ...
   unsigned numTargets, UInt16Vector& outTargets, UInt16Vector& outGroups)
{
   ...
   CONGFS_BUG_ON_DEBUG(groupsIter->second.empty(), "found group without targets");
