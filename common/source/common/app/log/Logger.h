...
namespace congfs { namespace logging {
...
#define LOG_CTX_TOP_L(Topic, Level, Context, Line, Message, ...) \
   do { \
      ...
      (void) _log_items; \
      congfs::logging::LogInfos _log_line; \
      const congfs::logging::SystemError sysErr; (void) sysErr; \
      const congfs::logging::InBase<std::hex> hex; (void) hex; \
      const congfs::logging::InBase<std::oct> oct; (void) oct; \
      _log_line << Message; \
      LOG_CTX_TOP_L_ITEMS( \
