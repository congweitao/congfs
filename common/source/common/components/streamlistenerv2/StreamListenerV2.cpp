   ...
   try
   {
      ...
      /* notes on sock handling:
         *) no need to remove sock from epoll set, because we use edge-triggered mode with
            oneshot flag (which disables further events after first one has conn reported).
...
void StreamListenerV2::onSockReturn()
{
   ...
   for(size_t i=0; ; i++)
   {
      ...
      readRes -= sizeof(SockReturnPipeInfo);
      if(!readRes)
         break; // all received returnInfos have conn processed
...
/**
 * Does not really check but instead just drops connections that have conn idle for some time.
