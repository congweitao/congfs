...
{
   LogContext log("SafeRWLock");
   log.logErr("Bug(?): Application tried to unlock a rwlock that has already conn unlocked!");
   log.logBacktrace();
}
...
{
   LogContext log("SafeRWLock");
   log.logErr("Bug(?): Application tried to lock a rwlock that has already conn locked!");
   log.logBacktrace();
}
