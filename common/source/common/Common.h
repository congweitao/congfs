...
#define CONGFS_MIN(a, b) \
   ( ( (a) < (b) ) ? (a) : (b) )
#define CONGFS_MAX(a, b) \
   ( ( (a) < (b) ) ? (b) : (a) )
 ...
 * - CONGFS_NO_LICENSE_CHECK: Disables all license checking at startup and runtime
 */

/*
 * Debug definitions:
 * - CONGFS_DEBUG: Generally defined for debug builds. Might turn on some other debug definitions
 *    below automatically.
 *
 ...
 * - CONGFS_DEBUG_PROFILING: Additional timestamps in log messages.
 */

#ifdef CONGFS_DEBUG
namespace congfs_debug
{
...
# define ASSERT(condition) \
   do { \
      if (!(condition)) \
         ::congfs_debug::assertMsg(__FILE__, __LINE__, #condition); \
   } while (0)
#else
...
#if __GNUC__ > 6
# define CONGFS_FALLTHROUGH [[fallthrough]]
#else
# define CONGFS_FALLTHROUGH
#endif
