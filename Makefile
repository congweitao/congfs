...
CONGFS_THIRDPARTY_OPTIONAL =
export CONGFS_THIRDPARTY_OPTIONAL
...
ifndef CONGFS_VERSION
  CONGFS_VERSION := $(shell git describe --tags --match '*.*' --abbrev=10)
  CONGFS_EPOCH := 19
else
  CONGFS_EPOCH := 20
endif
...
CONGFS_VERSION_RPM := $(subst -,_,$(CONGFS_VERSION))
CONGFS_VERSION_DEB := $(CONGFS_EPOCH):$(subst _,-,$(CONGFS_VERSION))
...
.PHONY: thirdparty
thirdparty:
	$(MAKE) -C thirdparty/build all $(CONGFS_THIRDPARTY_OPTIONAL)
...
$1-install: $1-all
	install -t $(DESTDIR)/$(PREFIX)/$2 -D \
		$1/build/congfs-$$(or $$(install_name),$1)
endef
...
common-install: common-all
	install -t $(DESTDIR)/$(PREFIX)/lib -D \
		common/build/libcongfs_ib.so

.PHONY: client-install
client-install: client
	install -t $(DESTDIR)/$(PREFIX)/lib/modules/$(KVER)/kernel/congfs -D \
		client_module/build/congfs.ko
...
event_listener-install: event_listener-all
	install -t $(DESTDIR)/$(PREFIX)/sbin -D \
		event_listener/build/congfs-event-listener
...
package-deb: clean
	...
	cd '$(PACKAGE_DIR)' && \
		...
		( \
			cd build; \
			sed -i -e 's/congfs (.*)/congfs ($(CONGFS_VERSION_DEB))/' debian/changelog; \
			sed -i -e 's/@DATE/$(shell date -R)/' debian/changelog; \
			debuild -eCONGFS_\* $(DEBUILD_OPTS) -us -uc -b \
		) && \
...
package-rpm: clean
	...
	tar --exclude $(PACKAGE_DIR) --exclude .git --exclude .ccache \
		-cf $(PACKAGE_DIR)/SOURCES/congfs-$(CONGFS_VERSION_RPM).tar .
	rpmbuild --clean -bb congfs.spec \
		--define '_topdir $(abspath $(PACKAGE_DIR))' \
		--define 'EPOCH $(CONGFS_EPOCH)' \
		--define 'CONGFS_VERSION $(CONGFS_VERSION_RPM)' \
		$(RPMBUILD_OPTS)
