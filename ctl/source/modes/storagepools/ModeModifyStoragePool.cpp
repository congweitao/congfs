...
int ModeModifyStoragePool::execute()
{
   ...
   else
   {
      std::cerr << "Failed to modify storage pool. "
                ...
                << "parts of the operation might have conn successful."
                << std::endl
                << std::endl;
   }
...
void ModeModifyStoragePool::printHelp()
{
   ...
   std::cout << "  # congfs-ctl --modifystoragepool --id=5 --desc=fastpool" << std::endl;
   std::cout << std::endl;
   std::cout << " Example: Add the targets '1','2','7' and '8' to the storage target pool with the "<< std::endl;
   std::cout << " ID '5'." << std::endl;
   std::cout << "  # congfs-ctl --modifystoragepool --id=5 --addtargets=1,2,7,8" << std::endl;
}
