...
int ModeSetPattern::execute()
{
   ...
   if (patternType == StripePatternType_BuddyMirror
         ...
         && !forceMode)
   {
      std::cerr << "Cannot set buddy mirroring when no groups have conn defined. "
...
void ModeSetPattern::printHelp()
{
   ...
   std::cout << "                               groups have conn defined." << std::endl;
   ...
   std::cout << "                               unmounted ConGFS. (Symlinks will not be resolved" << std::endl;
   ...
   std::cout << "  # congfs-ctl --setpattern --chunksize=1m --numtargets=4 /mnt/congfs/mydir" << std::endl;
   std::cout << std::endl;
   std::cout << " Example: Apply new pattern to all subdirectories of /mnt/congfs/mydir" << std::endl;
   std::cout << "  # find /mnt/congfs/mydir -type d | \\" << std::endl;
   std::cout << "     congfs-ctl --setpattern --chunksize=1m --numtargets=4 -" << std::endl;
}
