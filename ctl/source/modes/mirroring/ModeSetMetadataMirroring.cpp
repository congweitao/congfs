...
void ModeSetMetadataMirroring::printHelp()
{
   ...
   std::cout << " than the root directory after the directory has conn created." << std::endl;
   std::cout << std::endl;
   ...
   std::cout << "  # congfs-ctl --mirrormd" << std::endl;
}
      ...
      [] (const auto& data) {
            ...
            if (!clients.empty())
               std::cerr << "Error: There are still " << clients.size() << " clients activce.\n"
                            ...
                            "See congfs-ctl --listnodes --nodetype=client for more information."
                         << std::endl;
