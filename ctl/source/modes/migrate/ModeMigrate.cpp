...
int ModeMigrate::doExecute()
{
   ...
   {
      std::cerr << this->cfgSearchPath << " is not a ConGFS directory. Aborting." << std::endl;
      return APPCODE_RUNTIME_ERROR;
   }
...
void ModeMigrate::printHelpMigrate()
{
   ...
   std::cout << "     $ congfs-ctl --migrate --targetid=5 /mnt/congfs" << std::endl;
   std::cout << "   Migrate all files from targets in storage pool with ID \"2\" to targets from." << std::endl;
   std::cout << "   pool \"3\":" << std::endl;
   std::cout << "     $ congfs-ctl --migrate --storagepoolid=2 --destinationpoolid=3 /mnt/congfs" << std::endl;
}
...
void ModeMigrate::printHelpFind()
{
   ...
   std::cout << "  $ congfs-ctl --find --targetid=5 /mnt/congfs" << std::endl;
}
