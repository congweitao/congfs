...
int MigrateFile::runCreateIoctl(struct stat* fromStat, const char* symlinkTo)
{
   ...
   this->tmpName = CONGFS_TMP_FILE_NAME + StringTk::intToStr(System::getTID() );
...
bool MigrateFile::copyData(int fromFD, int tmpFD, struct stat* fromStatData)
{
   ...
   while (readSizeLeft) // read loop
   {
      ...
      {
         readSize = CONGFS_MIN(BUFFER_SIZE, readSizeLeft);
         wasReAligned = false;
      }
      else {
...
namespace
{
   ...
   void printError(const XAttrException& exception)
   {
      std::cerr << exception.what() << std::endl <<
         ...
         "file system with congfs-fsck and check the log files for error messages." << std::endl;
   }
}
