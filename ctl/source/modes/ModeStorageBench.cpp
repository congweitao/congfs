   ...
   else
   {
      switch (action)
      {
         case StorageBenchAction_START:
         {
            ...
            std::cout << "You can query the status with the --status argument of congfs-ctl.";
            std::cout << std::endl;
            std::cout << std::endl;
...
void ModeStorageBench::printHelp()
{
   ...
   std::cout << "  # congfs-ctl --storagebench --alltargets --write --blocksize=512K \\" << std::endl;
   std::cout << "     --size=200G --threads=10" << std::endl;
   std::cout << std::endl;
   std::cout << " Example: Query benchmark status/result of all targets" << std::endl;
   std::cout << "  # congfs-ctl --storagebench --alltargets --status" << std::endl;
}
