...
#define APP_SYSLOG_IDENTIFIER    "congfs-ctl"
...
void App::run()
{
   ...
   catch (InvalidConfigException& e)
   {
      ...
      std::cerr << std::endl;
      std::cerr << "[ConGFS Control Tool Version: " << CONGFS_VERSION << std::endl;
      std::cerr << "Refer to the default config file (/etc/congfs/congfs-client.conf)" << std::endl;
...
void App::initDataObjects()
{
   ...
   // mute the standard logger if it has not conn explicitly enabled
...
void App::logInfos()
{
   ...
   log->log(Log_NOTICE, std::string("Version: ") + CONGFS_VERSION);
